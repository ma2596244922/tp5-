...
.title span {
  ...
  height: 0.29rem;
  display: block;
  background: url(/media/sets/mobile/classify.png) left top no-repeat;
  background-size: 100%;
  float: right;
...
<link rel="canonical" href="{$smarty.server.REQUEST_SCHEME}://{$canonical_host}{$smarty.server.REQUEST_URI}">
    {$page_name}
    {-if in_array($page_name|default:'', ['home', 'product_list'])}
...
{foreach $products as $product}{if $product@index>=7}{continue}{/if}
            <li><a href="{$product|url:'enterprise_url_product'}" title="{$product.caption|escape}">
            		<amp-img src="{$product.head_image_id|url:'enterprise_url_image':$product.caption:'c'}" layout="responsive"  width="173" height="173" alt="{$product.caption|escape}"></amp-img>
            	</a>
