...
CREATE TABLE `enterprise_groups` (
  ...
  `created` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最近修改时间',
  `cnt` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '产品数量',
  `deleted` tinyint NOT NULL DEFAULT '0' COMMENT '已删除？',
  `path` text COMMENT '自定义路径',
  `path_sum` binary(16) COMMENT '自定义路径的摘要',
  `purl_prefix` varchar(15) NOT NULL DEFAULT '' COMMENT '产品URL前缀',
  `product_html_title` text COMMENT '产品页T',
  `product_meta_keywords` text COMMENT '产品页K',
  `product_meta_description` text COMMENT '产品页D',
  `html_title` text COMMENT 'T',
  `meta_keywords` text COMMENT 'K',
  `meta_description` text COMMENT 'D',
  `desc` text COMMENT '说明文本',
  `product_give_h1_to` tinyint NOT NULL DEFAULT '0' COMMENT '优先使用H1标签（0-产品标题；1-型号）',
  `product_video_uri` varchar(100) NOT NULL DEFAULT '' COMMENT '产品页视频地址',
  `product_video_cover_uri` varchar(100) NOT NULL DEFAULT '' COMMENT '产品页视频封面图',
  `product_video_duration` varchar(10) NOT NULL DEFAULT '' COMMENT '产品页视频时长',
  `ranking` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '排序',
  PRIMARY KEY (`id`),
  KEY `idx_get_by_site` (`site_id`, `deleted`),
...
CREATE TABLE `enterprise_products` (
  ...
  `locale` varchar(20) NOT NULL DEFAULT '' COMMENT '语种名称',
  `created` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最近修改时间',
  `deleted` tinyint NOT NULL DEFAULT '0' COMMENT '已删除？',
  `tags` varchar(255) NOT NULL DEFAULT '' COMMENT '关键词（逗号分隔）',
  `brand_name` varchar(100) NOT NULL DEFAULT '' COMMENT '品牌',
  `model_number` varchar(100) NOT NULL DEFAULT '' COMMENT '型号',
  `certification` varchar(100) NOT NULL DEFAULT '' COMMENT '认证证书',
  `place_of_origin` varchar(100) NOT NULL DEFAULT '' COMMENT '原产地',
  `min_order_quantity` varchar(100) NOT NULL DEFAULT '' COMMENT '最小起订量',
  `price` varchar(100) NOT NULL DEFAULT '' COMMENT '价格',
  `payment_terms` varchar(100) NOT NULL DEFAULT '' COMMENT '付款方式',
  `supply_ability` varchar(100) NOT NULL DEFAULT '' COMMENT '供货能力',
  `delivery_time` varchar(100) NOT NULL DEFAULT '' COMMENT '发货期限',
  `packaging_details` varchar(100) NOT NULL DEFAULT '' COMMENT '常规包装',
  `specifications` text COMMENT '产品属性（JSON）',
  `head_image_id` int unsigned NOT NULL DEFAULT '0' COMMENT '产品头图ID',
  `images` text COMMENT '产品图片（JSON）',
  `source_url` varchar(255) NOT NULL DEFAULT '' COMMENT '若产品是抓来的则保存原始URL',
  `html_title` text COMMENT 'T',
  `meta_keywords` text COMMENT 'K',
  `meta_description` text COMMENT 'D',
  `path` text COMMENT '自定义路径',
  `path_sum` binary(16) COMMENT '自定义路径的摘要',
  `embedded_video` text COMMENT '嵌入视频代码',
  `summary` varchar(300) NOT NULL DEFAULT '' COMMENT '描述摘要',
  PRIMARY KEY (`id`),
...
CREATE TABLE `enterprise_corporations` (
  ...
  `factory_address` varchar(100) NOT NULL DEFAULT '' COMMENT '厂址',
  `worktime` varchar(100) NOT NULL DEFAULT '' COMMENT '工作时间',
  `tel_wt` varchar(100) NOT NULL DEFAULT '' COMMENT '电话（工作时间内）',
  `tel_nwt` varchar(100) NOT NULL DEFAULT '' COMMENT '电话（非工作时间内）',
  `fax` varchar(100) NOT NULL DEFAULT '' COMMENT '传真',
  `skype` varchar(45) NOT NULL DEFAULT '' COMMENT 'Skype',
  `email` varchar(45) NOT NULL DEFAULT '' COMMENT '电子邮件地址',
  `yahoo` varchar(45) NOT NULL DEFAULT '' COMMENT 'Yahoo',
  `logo` int(10) unsigned NOT NULL DEFAULT '0' COMMENT 'LOGO图片ID',
  `business_type` varchar(45) NOT NULL DEFAULT '' COMMENT 'Business Type',
  `main_market` varchar(45) NOT NULL DEFAULT '' COMMENT 'Main Market',
  `brands` varchar(45) NOT NULL DEFAULT '' COMMENT 'Brands',
  `no_of_employees` varchar(45) NOT NULL DEFAULT '' COMMENT 'No. of Employees',
  `annual_sales` varchar(45) NOT NULL DEFAULT '' COMMENT 'Annual Sales',
  `year_established` varchar(45) NOT NULL DEFAULT '' COMMENT 'Year Established',
  `export_p_c` varchar(45) NOT NULL DEFAULT '' COMMENT 'Export p.c',
  `introduction` text COMMENT 'Introduction',
  `history` text COMMENT 'History',
  `service` text COMMENT 'Service',
  `our_team` text COMMENT 'Our Team',
  `qc_profile` text COMMENT 'QC Profile',
...
CREATE TABLE `enterprise_sites` (
  ...
  `updated` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最近修改时间',
  `start_year` smallint unsigned DEFAULT '0' NOT NULL COMMENT '起始年份',
  `common_fragment` text COMMENT '通用HTML代码片段',
  `contactnow_fragment` text COMMENT '询盘页HTML代码片段',
  `contactsave_fragment` text COMMENT '询盘成功页HTML代码片段',
  `product_cnt` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '产品总数',
  `index_products` text COMMENT '首页推荐产品ID清单（JSON）',
  `user_voices` text COMMENT '首页用户赠言（JSON）',
  `index_html_title` text COMMENT '首页T',
  `index_meta_keywords` text COMMENT '首页K',
  `index_meta_description` text COMMENT '首页D',
  `desc_4_inquiry_sender` text COMMENT '信息说明',
  `product_tdk_scope` tinyint NOT NULL DEFAULT '0' COMMENT '产品页TDK有效半径（0-全站；>0-分组ID）',
  `product_html_title` text COMMENT '产品页T',
  `product_meta_keywords` text COMMENT '产品页K',
  `product_meta_description` text COMMENT '产品页D',
  `contact_content` text COMMENT '联系页替代正文',
  `common_head_fragment` text COMMENT '通用HTML head代码片段',
  `product_default_image` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '产品默认图片ID',
  `common_bg_image` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '默认背景图片ID',
  `enable_inquiry_checking` tinyint unsigned NOT NULL DEFAULT '1' COMMENT '是否需要审核询盘(0/1)',
  `enable_mobile_sites` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '是否启用移动站(0/1)',
  `enable_https` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '是否启用HTTPS(0/1)',
  `offline` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '是否下线(0/1)',
  `purl_prefix` varchar(15) NOT NULL DEFAULT '' COMMENT '产品URL前缀',
  `gurl_prefix` varchar(15) NOT NULL DEFAULT '' COMMENT '产品分组URL前缀',
  `favicon` mediumblob COMMENT '收藏夹小图标数据',
  `default_lang_code` char(2) NOT NULL DEFAULT 'en' COMMENT '默认的语言代码',
  `lang_codes` text COMMENT '语种清单（JSON）',
  `enable_floating_widget` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '是否启用漂浮框(0/1)',
  `floating_widget_url` varchar(45) NOT NULL DEFAULT '' COMMENT '漂浮框URL',
  `disable_group_dk` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '是否禁用分组页D/K(0/1)',
  `translation_targets` text COMMENT '翻译目标语种清单（JSON）',
...
CREATE TABLE `oms_sites` (
  ...
  `wechat` VARCHAR(45) NOT NULL DEFAULT '' COMMENT '微信',
  `name` VARCHAR(45) NOT NULL DEFAULT '' COMMENT '姓名',
  `tel` varchar(45) NOT NULL DEFAULT '' COMMENT '联系电话',
  `sex` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '性别（0-男；1-女）',
  `csr` text COMMENT 'CSR',
  `key` text COMMENT 'Key',
  `vps_id` int(10) unsigned NOT NULL DEFAULT '0' COMMENT 'VPS ID',
  `industry_id` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '行业ID',
  `type` tinyint NOT NULL DEFAULT '0' COMMENT '网站类型（1-自营；2-赠送）',
  `created` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最近修改时间',
  `crawled` tinyint NOT NULL DEFAULT '0' COMMENT '是否抓取站点（0/1）',
  `enable_translator` tinyint NOT NULL DEFAULT '0' COMMENT '启用翻译器？（0/1）',
  PRIMARY KEY (`id`)
